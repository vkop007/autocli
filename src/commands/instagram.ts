...
`,
  ...
  command
    ...
    .option("--photos-only", "Only return photo stories")
    .option("--videos-only", "Only return video stories")
    .option("--account <name>", "Optional override for a specific saved Instagram session")
    .action(async (target, options, cmd) => {
      const ctx = resolveCommandContext(cmd);
      const logger = new Logger(ctx);
  ...
  command
    ...
    .option("--limit <number>", "Maximum number of posts to download (1-25, default: 5)", parseLimitOption)
    .option("--type <kind>", "Filter posts by media type: all, photo, video, reel, carousel", parseInstagramPostTypeOption)
    ...
    .action(async (target, options, cmd) => {
      const ctx = resolveCommandContext(cmd);
      const logger = new Logger(ctx);
      const spinner = logger.spinner("Downloading Instagram posts...");
      await runCommandAction({
        spinner,
        successMessage: "Instagram post download completed.",
        action: () =>
          adapter.downloadPosts({
            account: options.account,
            target,
            limit: options.limit,
            type: options.type,
  ...
  const batchCommand = command.command("batch").description("Run Instagram actions from a newline-delimited or JSON array input file");
  ...
  batchCommand
    ...
    .option("--account <name>", "Optional override for a specific saved Instagram session")
    .option("--limit <number>", "Maximum number of story items to download per profile (1-25, default: 5)", parseLimitOption)
    .option("--output-dir <path>", "Directory to write downloaded story files into")
    .option("--photos-only", "Only download photo stories")
    .option("--videos-only", "Only download video stories")
    .option("--fail-fast", "Stop after the first failed target")
  ...
  batchCommand
    ...
    .option("--account <name>", "Optional override for a specific saved Instagram session")
    .option("--limit <number>", "Maximum number of posts to download per profile (1-25, default: 5)", parseLimitOption)
    .option("--type <kind>", "Filter posts by media type: all, photo, video, reel, carousel", parseInstagramPostTypeOption)
    .option("--output-dir <path>", "Directory to write downloaded post files into")
    .option("--all", "Download every asset in carousel posts instead of only the first one")
    .option("--fail-fast", "Stop after the first failed target")
...
function parseInstagramPostTypeOption(value: string): "all" | "photo" | "video" | "reel" | "carousel" {
